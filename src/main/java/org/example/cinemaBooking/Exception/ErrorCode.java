...
public enum ErrorCode {
    ...
    CODE_REQUIRED(1071, "Code is required", HttpStatus.BAD_REQUEST),
    CODE_TOO_LONG(1072, "Code must be at most 50 characters", HttpStatus.BAD_REQUEST)
    ,NAME_TOO_LONG(1073, "Name must be at most 50 characters", HttpStatus.BAD_REQUEST),
    DESCRIPTION_TOO_LONG(1074, "Description must be at most 1000 characters", HttpStatus.BAD_REQUEST),
    DISCOUNT_TYPE_REQUIRED(1075, "Discount type is required", HttpStatus.BAD_REQUEST),
    DISCOUNT_VALUE_REQUIRED(1076, "Discount value is required", HttpStatus.BAD_REQUEST),
    DISCOUNT_VALUE_INVALID(1077, "Discount value must be greater than 0", HttpStatus.BAD_REQUEST),
    DISCOUNT_VALUE_FORMAT_INVALID(1078, "Discount value must be a valid number", HttpStatus.BAD_REQUEST),
    MIN_ORDER_VALUE_INVALID(1079, "Min order value must be greater than or equal to 0", HttpStatus.BAD_REQUEST),
    MIN_ORDER_VALUE_FORMAT_INVALID(1080, "Min order value must be a valid number", HttpStatus.BAD_REQUEST),
    MAX_DISCOUNT_INVALID(1081, "Max discount must be greater than or equal to 0", HttpStatus.BAD_REQUEST),
    MAX_DISCOUNT_FORMAT_INVALID(1082, "Max discount must be a valid number", HttpStatus.BAD_REQUEST),
    QUANTITY_INVALID(1083, "Quantity must be at least 1", HttpStatus.BAD_REQUEST),
    START_DATE_REQUIRED(1084, "Start date is required", HttpStatus.BAD_REQUEST),
    END_DATE_REQUIRED(1085, "End date is required", HttpStatus.BAD_REQUEST),
    MAX_USAGE_PER_USER_REQUIRED(1086, "Max usage per user is required", HttpStatus.BAD_REQUEST),
    MAX_USAGE_PER_USER_INVALID(1087, "Max usage per user must be at least 1", HttpStatus.BAD_REQUEST),
    PROMOTION_NOT_FOUND(1088, "Promotion not found", HttpStatus.NOT_FOUND),
    PROMOTION_ALREADY_EXISTS(1089, "Promotion already exists", HttpStatus.BAD_REQUEST),
    START_DATE_INVALID(1090, "Start date must not be in the past", HttpStatus.BAD_REQUEST),
    END_DATE_INVALID(1091, "End date must be after start date", HttpStatus.BAD_REQUEST),
    INVALID_DATE_RANGE(1092, "Start date must be before end date", HttpStatus.BAD_REQUEST),
    MAX_DISCOUNT_INVALID_FOR_PERCENTAGE(1093, "Max discount must be greater than or equal to discount value for percentage type", HttpStatus.BAD_REQUEST),
    INVALID_DISCOUNT_VALUE_FOR_PERCENTAGE(1094, "Discount value must be less than or equal to 100 for percentage type", HttpStatus.BAD_REQUEST),
    PROMOTION_ALREADY_DELETED(1095, "Promotion already deleted", HttpStatus.BAD_REQUEST),
    PROMOTION_EXPIRED(1096, "Promotion expired", HttpStatus.BAD_REQUEST),
    PROMOTION_NOT_STARTED(1097, "Promotion not started", HttpStatus.BAD_REQUEST),
    PROMOTION_OUT_OF_STOCK(1098, "Promotion out of stock", HttpStatus.BAD_REQUEST),
    PROMOTION_USAGE_LIMIT_EXCEEDED(1099, "Promotion usage limit exceeded", HttpStatus.BAD_REQUEST),
    QUANTITY_LESS_THAN_USED(1100, "Quantity must be greater than or equal to used quantity", HttpStatus.BAD_REQUEST),
    MAX_DISCOUNT_NOT_ALLOWED(1101, "Max discount is not allowed for fixed discount type", HttpStatus.BAD_REQUEST),;
